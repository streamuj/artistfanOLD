...
class Music extends BaseMusic
{
    ...
    public static function GetMusicListByTitleCount($title )
    {
         ...
         $sql = sprintf('SELECT count(m.id) as Count
				...
				INNER JOIN user as u ON (u.Id = m.user_id AND u.blocked=0)
                WHERE m.deleted = 0 ');
    ...
    {
         $sql = sprintf('SELECT m.id as Id,ma.id as AlbumId,ma.image as Image, m.title AS Title, m.track AS Track, m.track_preview as TrackPreview,u.id as UserId, u.band_name AS BandName, u.Name AS Name, u.Avatar AS Avatar
                FROM music as m  
				INNER JOIN music_album as ma ON (ma.id = m.album_id AND ma.deleted =0 AND m.deleted =0)
				INNER JOIN user as u ON (u.Id = m.user_id AND u.blocked=0)
                WHERE m.deleted = 0 ');
...
//                ->filterByDeleted(0);
	...
	public function ShowAlbumsByMusicPurchaseId($userId, $page = 0, $items_on_page = 0, $id=0, $trackList=0)
	{
		...
		LEFT JOIN (SELECT COUNT(music_purchase.id) AS totalTracks, album_id  
			FROM music_purchase 
			INNER JOIN music on music_id = music.id
			WHERE music_purchase.user_id = '.$userId.' GROUP BY album_id) mp on mp.album_id = music_album.id 
		...
		if($trackList)
		{
			foreach($all  as &$v)
			{
				$v['TrackList'] = Music::GetPurchaseMusicTrackByAlbumId($v['album_id'], $userId, 1);
			}
	...
	public static function GetPurchaseMusicTrackByAlbumId($albumId, $UserId, $active=0)
	{
	...
	public static function GetTrackListByalbumId($albumId, $userId)
	{
	
		$sql ='SELECT * FROM  music WHERE album_id ='.$albumId.' AND user_id='.$userId;
		$all = Query::GetAll($sql);
		return $all;
