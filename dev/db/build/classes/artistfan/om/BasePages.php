...
abstract class BasePages extends BaseObject  implements Persistent
{
	...
	public function setPagename($v)
	{
		...
		if ($this->pagename !== $v) {
			$this->pagename = $v;
			$this->modifiedColumns[] = PagesPeer::PAGENAME;
		}
	...
	public function setSortid($v)
	{
		...
		if ($this->sortid !== $v) {
			$this->sortid = $v;
			$this->modifiedColumns[] = PagesPeer::SORTID;
		}
	...
	public function setStory($v)
	{
		...
		if ($this->story !== $v) {
			$this->story = $v;
			$this->modifiedColumns[] = PagesPeer::STORY;
		}
	...
	public function setPdate($v)
	{
		...
		if ($this->pdate !== $v) {
			$this->pdate = $v;
			$this->modifiedColumns[] = PagesPeer::PDATE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->pagename = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->sortid = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->story = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(PagesPeer::PAGENAME)) {
			$modifiedColumns[':p' . $index++]  = '`PAGENAME`';
		}
		if ($this->isColumnModified(PagesPeer::SORTID)) {
			$modifiedColumns[':p' . $index++]  = '`SORTID`';
		}
		if ($this->isColumnModified(PagesPeer::STORY)) {
			$modifiedColumns[':p' . $index++]  = '`STORY`';
		}
		if ($this->isColumnModified(PagesPeer::ACTIVE)) {
			$modifiedColumns[':p' . $index++]  = '`ACTIVE`';
		}
		if ($this->isColumnModified(PagesPeer::PDATE)) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
	{
		...
		$result = array(
			...
			$keys[3] => $this->getSortid(),
			$keys[4] => $this->getStory(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setSortid($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setStory($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PagesPeer::PAGENAME)) $criteria->add(PagesPeer::PAGENAME, $this->pagename);
		if ($this->isColumnModified(PagesPeer::SORTID)) $criteria->add(PagesPeer::SORTID, $this->sortid);
		if ($this->isColumnModified(PagesPeer::STORY)) $criteria->add(PagesPeer::STORY, $this->story);
		if ($this->isColumnModified(PagesPeer::ACTIVE)) $criteria->add(PagesPeer::ACTIVE, $this->active);
		if ($this->isColumnModified(PagesPeer::PDATE)) $criteria->add(PagesPeer::PDATE, $this->pdate);
