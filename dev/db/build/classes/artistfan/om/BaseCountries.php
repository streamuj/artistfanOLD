...
abstract class BaseCountries extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseCountries object.
	 * @see        applyDefaults()
	 */
	...
	public function setPcode($v)
	{
		...
		if ($this->pcode !== $v) {
			$this->pcode = $v;
			$this->modifiedColumns[] = CountriesPeer::PCODE;
		}
	...
	public function setSortid($v)
	{
		...
		if ($this->sortid !== $v) {
			$this->sortid = $v;
			$this->modifiedColumns[] = CountriesPeer::SORTID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->iso2 = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->pcode = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->sortid = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->iso3 = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(CountriesPeer::PCODE)) {
			$modifiedColumns[':p' . $index++]  = '`PCODE`';
		}
		if ($this->isColumnModified(CountriesPeer::SORTID)) {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`SORTID`':
						...
						break;
					case '`ISO3`':
						$stmt->bindValue($identifier, $this->iso3, PDO::PARAM_INT);
						break;
				}
			}
			$stmt->execute();
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIso2($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setName($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setPcode($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CountriesPeer::PCODE)) $criteria->add(CountriesPeer::PCODE, $this->pcode);
		if ($this->isColumnModified(CountriesPeer::SORTID)) $criteria->add(CountriesPeer::SORTID, $this->sortid);
	...
	{
		$criteria = new Criteria(CountriesPeer::DATABASE_NAME);
		$criteria->add(CountriesPeer::ISO2, $this->iso2);
	...
	/**
	 * Generic method to set the primary key (iso2 column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setIso2(NULL); // this is a auto-increment column, so set to default value
		}
	}
