...
abstract class BaseState extends BaseObject  implements Persistent
{
	...
	public function setStateCode($v)
	{
		...
		if ($this->state_code !== $v) {
			$this->state_code = $v;
			$this->modifiedColumns[] = StatePeer::STATE_CODE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->state_code = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = StatePeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(StatePeer::STATE_CODE)) {
			$modifiedColumns[':p' . $index++]  = '`STATE_CODE`';
		}
		if ($this->isColumnModified(StatePeer::STATE_NAME)) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setStateCode($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(StatePeer::STATE_CODE)) $criteria->add(StatePeer::STATE_CODE, $this->state_code);
	...
	{
		$criteria = new Criteria(StatePeer::DATABASE_NAME);
		$criteria->add(StatePeer::STATE_CODE, $this->state_code);
	...
	/**
	 * Generic method to set the primary key (state_code column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		if ($makeNew) {
			$copyObj->setNew(true);
			$copyObj->setStateCode(NULL); // this is a auto-increment column, so set to default value
		}
	}
