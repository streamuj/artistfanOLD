...
class Home extends BaseHome { 
	...
	{		
			...
			FROM home as h 
			LEFT JOIN user as u ON u.id = h.artist_id 
	...
	{		
			$sql = 'SELECT  count(h.home_id) as cnt FROM home as h  LEFT JOIN user as u ON u.id = h.artist_id WHERE h.artist_id !=0 ';
	...
	{		
			...
			FROM home as h 			
			LEFT JOIN video as v ON h.video_id=v.id 
	...
	{		
			...
			FROM home as h 			
			LEFT JOIN event as e ON h.event_id=e.id 
