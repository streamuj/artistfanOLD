...
LEFT JOIN user as u ON u.id = ma.user_id 
WHERE ma.active =1 AND ma.pdate < '.$dstart.' AND ma.pdate >= '.$dend.' AND ifnull(Tracks, 0)  > 0 AND ma.date_release <= "'.$MusicDate.'"';
...
LEFT JOIN user as u ON u.id = ma.user_id 
WHERE ma.active =1 AND ma.pdate < '.$dstart.' AND ma.pdate >= '.$dend.' AND ifnull(totalTrack, 0)  > 0 AND ma.date_release <= "'.$MusicDate.'"';
...
//deb($res);					
	...
	{	
			...
			WHERE m.deleted = 0 AND m.active =1 AND ma.is_single =1 AND m.status = 0 group by ma.id order by count desc, mp.with_all_album desc';
	...
	public static function GetHotSinglesAlbumListCount()
	{
		$sql ='SELECT mp.with_all_album as Id
			...
			WHERE m.deleted = 0 AND m.active =1 AND ma.is_single =1 AND m.status = 0 group by ma.id';
