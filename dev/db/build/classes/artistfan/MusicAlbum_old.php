...
class MusicAlbum extends BaseMusicAlbum {
     ...
     * Get album by Id
     */
    public static function GetAlbum( $id, $artist_info = 0, $add_info = 0 )
    {
...
/*            if ($album['Image'])
	...
	{
		$qry = 'SELECT sum(price) as totalPrice FROM music WHERE album_id = '. $albumId;
    ...
    public static function GetAlbumList( $user_id, $page = 0, $items_on_page = 0, $active = 1, $add_info = 0, $puchaseInfo = 0)
    {
    ...
    public static function GetRecentAlbumList( $active = 1, $page = 1, $items_on_page = 0, $add_info = 0, $userId,  $mCache = '', $user_id = 0)
    {
...
//print_r( date('d-m-Y H:i:s', $dstart).'dsd<br>');
//print_r( date('d-m-Y H:i:s', $dend).'dsd<br>');
		...
		if($userId)
		{
			foreach($albums as &$v)
			{
				 ...
				 $IsFollow1 = UserFollow::GetFollow($v['UserId'], $userId);
				 if(empty($IsFollow1))
				 {
					 $v['follower']  = 1;
				 }
    ...
    {
        $tracks = MusicQuery::create()->select(array('Id', 'UserId', 'Track', 'TrackPreview', 'Genre', 'Price', 'Title'))->filterByAlbumId($album_id)->find()->toArray();
	...
	public static function GetBestSellerAlbumList($items_count, $userId, $mCache = '', $user_id = 0)
	{	
	        //get from cache
	...
	public static function GetHotSinglesAlbumList($items_count, $userId,  $mCache = '', $user_id = 0)
	{	
	        //get from cache
        
		if (!empty($mCache))
        {
            $bs_album = $mCache->get('hs_track_onexplore' . $user_id, 12*3600);
            if (!empty($bs_album))
            {
                return @unserialize($bs_album);
            }
			...
			$sql ='SELECT mp.with_all_album as Id, count(mp.with_all_album) as count, m.price AS Price, m.user_id as UserId, ma.image as Image, m.title as Title, m.album_id as AlbumId, u.name as Name, u.first_name as FirstName, u.last_name as LastName, u.band_name as BandName 			 			
			FROM music_purchase as mp 
	   ...
	   //save to cache
        if (!empty($mCache))
        {
            $mCache->set('hs_track_onexplore' . $user_id, serialize($bs_album), 12*3600);
        }
	...
	public static function GetArtistAlbumList($artistUserId, $LoginUserId, $page, $items_count)
    {	
		 ...
		 $sql = 'SELECT  DISTINCT ma.id AS Id, ma.title AS Title, ma.descr AS Descr, ma.date_release AS DateRelease, ma.image AS Image, ma.price AS Price,  ma.genre AS Genre, ma.label AS Label,  ma.is_single AS IsSingle, ma.album_pay_more AS AlbumPayMore,  user.id AS UserId, user.first_name AS FirstName, user.last_name AS LastName, user.band_name AS BandName, user.name AS Name, music_purchase.with_all_album as purchased, totalTrack 
					 ...
					 LEFT JOIN (select count(music.id) AS totalTrack, album_id FROM music GROUP BY album_id) music_track on music_track.album_id = ma.id
