...
class Security_User
{
    ...
    public function __construct( $mSession )
    {
        ...
        //Logout after ... seconds, 0 - disabled
        $this->mLogoutAfter = SESSION_TIME;
    }
    ...
    public function CheckAuth()
    {
		...
		if(!$_SESSION['system_uid'] && (($system_login && $system_pass)|| $fb_id || $tw_access))
        { 
        ...
        else
        {
            ...
            {	
               // if last reload time < N minutes - logout
                ...
                {
				 	UserQuery::create()->Select(array('Id'))->filterById($row['Id'])->Update(array('IsOnline' => 0));
				...
				$memTrack = unserialize($row['Members']);			
				$row['Members'] = $memTrack[0];
				$row['Tracks'] = $memTrack[1];
                ...
                $this->mTwitterId = $this->mSession->Get('twitter_id');
				 $this->mSession->Del('nactive');

                // update last login && last reload to current date
                UserQuery::create()->Select(array('Id'))->filterById($row['Id'])->Update(array('LastReload' => mktime(), 'Ip' => real_ip(), 'IsOnline' => 1));
