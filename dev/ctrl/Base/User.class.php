...
//									$wallId = Wall::Add( $userDetail['Id'], $userDetail['Id'], $mesg, $image, '', 0, $this->mCache );																		
                ...
                if ($userDetail['Email'] == $fm['email'])
                {
					...
					$this->mSmarty->assign('FirstName', $fm['first_name']);
					$this->mSmarty->assign('LastName', $fm['last_name']);
                    $this->mSmarty->assign('name', $name);
...
/*		echo "<pre>img :";
		echo $orginalImage.'<br>';
		echo 'Rand :'.$rand_id.'<br>';
    ...
    public function TwtGetAuthUrl()
    {
		...
		$content = $con->get('account/verify_credentials');
		deb($content);*/
		//$this->mlObj['mSession']->set('twitter_id', $request_token['user_id']);
		//deb($request_token);
