...
/*            if (!$fm['EventType'] || !in_array($fm['EventType'], array(1, 2, 3)))
	...
	public function PricedTracks()
	{
		...
		// $delete_music_id = _v('delete_music_id', '');	
		$page = _v('page', 1);
		$pcnt = 10;	
		$id = _v('id', 0);	
		if($id)
    ...
    {	
		...
		if($delete_video_id)
		{
			...
			$videos = Video::GetVideoList($this->mUser->mUserInfo['Id'], 1, $page, $pcnt, '', $video_type);
			...
			//$res['pagging'] = $mpg->Make2($this->mSmarty, 0, 1); 
			$res['pagging'] = $mpg->Make($this->mSmarty, 1);
            $res['page'] = $page;
        ...
        {
            //video list			
	//		$videos = Video::GetVideoList($this->mUser->mUserInfo['Id'], 1, 0, 0, '', $video_type);
			$videos = Video::GetVideoList($this->mUser->mUserInfo['Id'], 1, $page, $pcnt, '', $video_type);
			$rcnt = $videos['rcnt'];
			$videos = $videos['list'];
			include_once 'model/Pagging.class.php';
        ...
        else
        {
			...
			$musicVideo = Video::GetVideoList($this->mUser->mUserInfo['Id'], 1);
			
			
			$this->mSmarty->assignByRef('musicVideo', $musicVideo['list']);
			
			$streamVideo = Video::GetVideoList($this->mUser->mUserInfo['Id'], 1, '', '', '', RE_LIVE_STREAM);
				...
				/*	if($video_pr)
					{
						...
						$fn_preview = substr(md5(uniqid(mt_rand(), true)), 0, 10). date("hm") . $ext;
						//substr(md5(mktime()), 5, 10) . date("hm") . $ext;
						...
						$gPdo = new PDO(VBASE, $conf['datasources']['artistfan']['connection']['user'], $conf['datasources']['artistfan']['connection']['password']);
						$gPdo->query('SET CHARACTER SET utf8');
						$gPdo->query("INSERT INTO " . VTABLE . " (from_fname, to_fname, cdate, save_frames, in_proc, pdate) VALUES ('" . $fn_preview . "', '" . $fn_preview . ".mp4', 0, '', 0, 0)");
						$gPdo = null;
					...
					*/
					...
					{      		     
						foreach($follow as $followers)
						{
							$name = $followers['BandName'] ? $followers['BandName'] : $followers['FirstName'].' '.$followers['LastName'];						
							$this->mSmarty->assign('name', $name);  
							$this->mSmarty->assign('musicalbum', $fm);										
					...
					/*	if($video_pr && !empty($video['VideoPreview']))
						{
							$ftpServer = new Ftp(STREAMING_FTP_HOST, STREAMING_FTP_USER, STREAMING_FTP_PASSWORD);							
							$flashFile = '/flash/u/'. $video['UserId'].'/'.$video['VideoPreview'];
							$mobileFile = '/mobile/u/'. $video['UserId'].'/'.$video['VideoPreview'];
							$ftpServer->connect();
							$ftpServer->delete($flashFile);
							$ftpServer->delete($mobileFile);
...
//			uni_redirect(PATH_ROOT . 'artist/broadcasting');
	...
	public function getFirstFlowTime()	
	{
...
//deb($fans);
	...
	{
		 $this->mSmarty->display('mods/profile/edit_artist/artist_invitation.html');
	}    
	
	// google invitation
	public function googleInvite()
	{
		 include_once 'model/contacts/google/oauth/index.php';
	...
	public function musicUploadStep3()
	{
			...
			if($PublishId)
			{
				    $albumInfo = MusicAlbum::GetAlbum($PublishId, 0, 1);
					...
					//start here to send mail using id $PublishId;
//start					
					$follow 	 = UserFollow::GetFollowersUserList($this->mUser->mUserInfo['Id'], USER_ARTIST);
					$fellow_fans = UserFollow::GetFollowersUserList($this->mUser->mUserInfo['Id'], USER_FAN);
					...
					//end					
					//setRedirect(ROOT_HTTP_PATH.'artist/music');			
				 	uni_redirect( PATH_ROOT . 'artist/music');
			}
