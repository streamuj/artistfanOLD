...
{  
	...
	public function AlbumExistsInCart()
	{
      $redirect = $this->GetRedirectUrl();
		
  	  if($_REQUEST['ajaxMode'])
	  {			
			//echo $this->mUser->mUserInfo['Id'];
			$albumId = $_REQUEST['a_id'];
			$res['data'] = Music::GetMusicAlbumWithPurchase($this->mUser->mUserInfo['Id'],$albumId);			
			$res['q'] == OK;
			echo json_encode($res);
			exit(); 
	  } 
	...
	//deb($IsFollow1);
        ...
        if (!$id)
        {
            ...
            else
            {
				...
				//$album = Music::GetMusicList($ui['Id'], 0, 1, 0, 0, 0, 1);	
    ...
    private function PlayMusic( &$ui )
    {
		...
		if ($id)
        {
            ...
            {
				uni_redirect( PATH_ROOT . 'u/'.$ui['Name'].'/music' );
            }
            if ($this->mUser->IsAuth() && $ui['Id'] != $this->mUser->mUserInfo['Id'])
            {
            ...
            $this->mSmarty->display('mods/profile/show_artist/music_one.html');
        } else {
			uni_redirect( PATH_ROOT . 'u/'.$ui['Name'].'/music' );
		}
	...
	*/
	...
	public function UpdateFbTwRequest()
	{
		...
		if (empty($fbTwFollower))
		{
			...
			$mesg	=	 '<a href="/u/'.$ui['Name'].'" class="icon_01">'.$userName.'</a> is now following '.$this->mUser->mUserInfo['FirstName'].'';
