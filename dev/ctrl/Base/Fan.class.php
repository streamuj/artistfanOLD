...
class Base_Fan extends Base
{
    ...
    public function Wallet()
    {
        ...
        /*if (!empty($_POST['do']))
        {
            $points = abs(_v('points', 0));
            $errs = array();

            if (!empty($points))
            {
                $sum = $this->mUser->mUserInfo['Wallet'] + $points;
                //TODO :: Insert payment here
                User::UpdateWallet($this->mUser->mUserInfo['Id'], $sum);
                //for fan transaction history
                Payout::PayoutMoney($this->mUser->mUserInfo['Id'], 1, 0, $points, $sum, $this->mUser->mUserInfo['Id'], 1, array('type' => 'points'));
                //for generate redurect url need new value of user wallet
                $this->mUser->mUserInfo['Wallet'] = $this->mUser->mUserInfo['Wallet'] + $points;
                $redirect = $this->GetRedirectUrl();
                if($redirect)
                {
                    uni_redirect(PATH_ROOT . $redirect);
                }
                uni_redirect(PATH_ROOT . 'fan/wallet');
            }
            else
            {
                $errs['points'] = 'Please specify points';
            }
            $this->mSmarty->assignByRef('errs', $errs);
        }*/


        $this->mSmarty->display('mods/profile/edit_fan/wallet.html');
        exit();
    }
    ...
    {
        $res = array('q' => ERR, 'errs' => array(), 'redirect' => '', 'ballance' => 0);
        ...
        if(!empty($fm))
        {
            ...
            if(empty($fm['amount']))
            {
                $res['errs']['amount'] = PLEASE_SPECIFY_AMOUNT;
            }
            else if(!is_numeric($fm['amount']))
            {
                $res['errs']['amount'] = PLEASE_SPECIFY_CORRECT_AMOUNT;
            }
            else
            {
                $fm['amount'] = abs((float)$fm['amount']);
            }
            if(empty($fm['cardHolder']))
            {
                $res['errs']['cardHolder'] = PLEASE_SPECIFY_CARD_HOLDER_NAME;
            }
            if(empty($fm['countryId']) || !is_numeric($fm['countryId']))
            {
                $res['errs']['countryId'] = PLEASE_SELECT_COUNTRY;
            }
            if(empty($fm['avsAddress']))
            {
                $res['errs']['avsAddress'] = PLEASE_SPECIFY_STREET_ADDRESS;
            }
            else if(StringLen($fm['avsAddress']) > 20)
            {
                $res['errs']['avsAddress'] = MAX_STREET_ADDRESS_LENGTH_20_CHARACTERS;
            }
            if(empty($fm['avsZip']))
            {
                $res['errs']['avsZip'] = PLEASE_SPECIFY_ZIP_CODE;
            }
            else if(StringLen($fm['avsZip']) > 9)
            {
                $res['errs']['avsZip'] = MAX_ZIP_CODE_LENGTH_9_CHARACTERS;
            }
            if(empty($fm['cardNumber']))
            {
                $res['errs']['cardNumber'] = PLEASE_SPECIFY_CARD_NUMBER;
            }
            else if(StringLen($fm['cardNumber']) > 16)
            {
                $res['errs']['cardNumber'] = MAX_CARD_NUMBER_LENGTH_16_CHARACTERS;
            }
            if(empty($fm['cardExpiryMonth']) || empty($fm['cardExpiryYear']))
            {
                $res['errs']['cardExpiry'] = PLEASE_SPECIFY_CARD_EXPIRATION_DATE;
            }
            else if(!empty($fm['cardExpiryMonth']) && ((int)$fm['cardExpiryMonth'] < 1 || (int)$fm['cardExpiryMonth'] > 12))
            {
                $res['errs']['cardExpiry'] = PLEASE_SPECIFY_CORRECT_CARD_EXPIRATION_DATE;
            }
            else if(!empty($fm['cardExpiryYear']) && (int)$fm['cardExpiryYear'] < date('Y'))
            {
                $res['errs']['cardExpiry'] = PLEASE_SPECIFY_CORRECT_CARD_EXPIRATION_DATE;
            }
            if(empty($fm['cardCvv']))
            {
                $res['errs']['cardCvv'] = PLEASE_SPECIFY_CARD_VALIDATION_CODE;
            }
            if(empty($res['errs']))
            {
                ...
                /*
				require_once 'libs/Payvision/Payvision.class.php';
                $payv = new Payvision(PAYVISION_MEMBER_ID, PAYVISION_MEMBER_GUID);
                $payv->sendRequest($fm);
                $response = $payv->getResponse();
				*/
                ...
                if ($response->approved) {
                    ...
                    $tid = Payout::PayoutMoney($this->mUser->mUserInfo['Id'], 1, 0, $fm['amount'], $sum, $this->mUser->mUserInfo['Id'], 1, array('type' => 'points', 'TransactionId' => $response->transaction_id, 'TransactionGuid' => $response->invoice_number));
            ...
            {
                $res['errs']['common'] = ERRORS_WERE_FOUND_WITH_YOUR_POINTS_BUY_PLEASE_CORRECT_THE_HIGHLIGHTED_FIELDS_BELOW;
            }
        }
    ...
    public function WalletHistory()
    {
        ...
        if ($rcnt)
        {
            ...
            $link = PATH_ROOT . 'fan/wallethistory' . ($sort ? '?sort=' . $sort : '');
        ...
        $this->mSmarty->display('mods/profile/edit_fan/wallet_history.html');
        exit();
    }
