...
$mSmarty->assign('SUB_DOMAIN', SUB_DOMAIN);
	...
	$mAlbum = MusicAlbum::GetMusicAlbumEmailSendList();
//deb($mAlbum);
	include BPATH.'libs/Phpmailer_v5.1/class.phpmailer.php';

	foreach($mAlbum as $val)
	{
			...
			{      		     
				foreach($follow as $followers)
				{
					...
					$message = $mSmarty->fetch('mails/musicalbum_add_notification.html');
			...
			MusicAlbum::UpdateMusicAlbumEmailSend($val['Id']);				
