...
Users.prototype =
{
    ...
    Follow: function(user_id_follow) {		
		$('#follow').html('<img src="'+subDomain+'si/wait.gif"> ');
        $.ajax({
            type:'POST',
            dataType:'json',
            data:{
   ...
   ConnectFollow: function(user_id_follow,username,url){	   
    ...
    InitRegistration: function() {
        ...
        $('#accept').click(function(){	
           var anumgenres = $('#accept:checkbox').filter(':checked').length;
            if(anumgenres == 0)
            {

                oErrors.SetBadFld('accept', 'You must accept terms and condtions');
                err = 1;
            }
            else
            {
                oErrors.SetOkFld('accept');
                err = 0;
            }
        });
        ...
        $('#reg_form').submit(function() {
           ...
           var numgenres = $('#accept:checkbox').filter(':checked').length;
            if(numgenres == 0)
            {
