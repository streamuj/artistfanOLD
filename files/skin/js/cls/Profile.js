...
Profile.prototype =
{
    ...
    EditEvent: function() 
    {
		...
		if($('#event_image').attr('src') == subDomain+'si/nophoto.jpg')
		{
		...
		if (price<0.99)
		{
			errs['Price'] = 'Please price event fee minimum $0.99 or more.';
			er = 1;
		}			
		if (er)
        {
    ...
    EditVideo: function() {
        ...
        $('#err_Title, #err_Price, #VideoLink, #err_VideoDate').html('');
        errs = {};  
        var er = 0;
 ...
 DelPrimary: function(delet)
  {
   ...
   SlideLinkSelectMedia: function(m_type)
	...
	SaveAlbum: function()
	{
		$('#err_Title', '#err_Price', '#err_Descr', '#err_PublishDate').removeClass('error').html('');
		...
		if (jQuery.trim($("#DateRelease").val()) == "")
        {
            errs['PublishDate'] = 'Please specify publish Date';
            er = 1;
        }
        ...
        else
        {
			...
			$.ajax({
				...
				checkResponseRedirect(response);
					...
					$('#albumPrice').html(response.TrackPrice);
					$('#trackCount').html(response.trackCount);
				}
			});
        }
	...
	UpdateMusicOrder:function(Id)
	{
		$.ajax({
				url: '/base/artist/updateMusicSaveOrder',
				type:'post',
				data: {'Id': Id},
				dataType:'json',
				success: function(response)
