	 ...
	 payMoreAlbumBuyMusicDialogClear: function()
    {
	   ...
	   PayMoreUrlMusicAlbum: function()
        ...
        if(price > 0)
        {
            ...
            $( "#buy_music_dialog" ).find('.price strong').html('<input type="text" style="width:85px;height:25px;" class="btn1"  name="payMore" id="payMore" >');
                ...
                if (data.q == 'ok')
                {
                    ...
                    for (var i in data.list)
                    {
                        s += '<li class="viewer" vid="'+data.list[i]['Id']+'">' +
                                  '<a href="/u/'+data.list[i]['Name']+'"><img src="' + (data.list[i]['Avatar'] != '' ? subDomain+'images/avatars/x_' + data.list[i]['Avatar'] : '/i/ph/user-48x48.png') + '" width="48" height="48" alt="" /></a>' +
                              '</li>';
                    }
